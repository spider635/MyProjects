...
async def prop_task(queue, index, cache, f_cache, t_out):
    while not queue.empty() and not prop_stat.done:
        r = await queue.get()
        a = index[r['id']] = cache[r['id']] = await asyncio.get_event_loop().run_in_executor(None, get_prop, r['id'],
                                                                                             r['ts'])
        if a == None:
            prop_stat(err_inc=1)
        else:
...
def prop_stat(toc_set=None, toc_inc=0, num_set=None, num_inc=0, num_tot=0, err_set=None, err_inc=0, init=False):
    ...
    print_progress(
        f'    files: {prop_stat.toc[0]}/{prop_stat.toc[1]} records: {prop_stat.num[0]}/{prop_stat.num[1]} errors: {prop_stat.err} results: {prop_stat.total}')
...
def update_lists():
    ...
    ret = {e[0].text: e[1].text for e in
           ET.fromstring(requests.get(f'{u}?ts={time.time()}', headers={"User-Agent": USER_AGENT_HEADER}).text)}
...
def main():
    ...
    with open(args.output, 'w+', encoding='utf-8-sig', newline='') as f_out:
        ...
        t_out.writerow(
            ['ID', 'Type', 'Address', 'State', 'Suburb', 'Postcode', 'Status', 'Buy', 'Rent', 'Bathroom', 'Bedroom',
             'Carspace', 'Size',
